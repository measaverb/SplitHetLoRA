...
class ClientGPT2Model(nn.Module):
    def __init__(self, config):
        super(ClientGPT2Model, self).__init__()
...
class ServerGPT2Model(nn.Module):
    def __init__(self, config):
        super(ServerGPT2Model, self).__init__()
        self.n_layer = config.n_layer
        self.n_embd = config.n_embd
        self.n_vocab = config.vocab_size
    ...
    def forward(self, hidden_states, presents, input_shape, past=None, len_past=None):
        if past is None:
            # past_length = 0
            past = [None] * len(self.h)
        # elif len_past is None:
...
class ClientGPT2LMModel(nn.Module):
    def __init__(self, config):
        super(ClientGPT2LMModel, self).__init__()
        self.client_transformer = ClientGPT2Model(config)

    def set_tied(self):
        self.lm_head.set_embeddings_weights(self.client_transformer.wte.weight)
    ...
    def forward(
        ...
        hidden_states_client, presents_client = self.client_transformer(
        ...
        return (
            ...
            presents_client,
            self.client_transformer.state_dict(),
        )
    ...
    def load_weight(self, state_dict):
        ...
        for n, p in self.client_transformer.named_parameters():
            if n not in state_dict:
                state_dict[n] = p

        self.client_transformer.load_state_dict(state_dict, strict=False)


class ServerGPT2LMModel(nn.Module):
    def __init__(self, config):
        super(ServerGPT2LMModel, self).__init__()

        self.server_transformer = ServerGPT2Model(config)
        self.lm_head = GPT2LMHead(self.server_transformer.wte.weight, config)
        self.apply(self._init_weights)

    def set_tied(self):
        self.lm_head.set_embeddings_weights(self.server_transformer.wte.weight)
    ...
    def forward(
        ...
        hidden_states_server, presents_server = self.server_transformer(
            hidden_states_client, presents_client, input_ids_shape
        )
    ...
    def load_weight(self, state_dict):
        ...
        for n, p in self.server_transformer.named_parameters():
            if n not in state_dict:
                state_dict[n] = p
